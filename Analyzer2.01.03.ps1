...
.Notes  
    ...
    06/25/2025 - 2.01.03 - added support for Dock drivers - NOT firmware
...
.Parameters
    ...
    -CleanOutput            -- [switch] only show Softpaqs that require action
...
$ScriptVersion = '2.01.03' # version of the script
...
$Script:Config = @{
    ...
    Paths = @{
        LogFile = "Analyzer-$((Get-Date).ToString('yyyyMMdd-HHmm')).log"
        CsvFile = "Analyzer-$((Get-Date).ToString('yyyyMMdd-HHmm')).csv"
        JsonFile = "Analyzer-$((Get-Date).ToString('yyyyMMdd-HHmm')).json"
    }
    OSVersionMap = @{
...
if ( -not $CleanOutput ) { TraceLog -Message "Analyzer: $($ScriptVersion) -- $($startTime)" }
# Log the command line
if ( $DebugOut ) { TraceLog -Message "-- Script executed as: $($MyInvocation.Line)" }
...
function Initialize-Environment {
    ...
    Try {
        ...
        if ( -not $CleanOutput ) { TraceLog -Message "-- Analyzing platform: [$($ie_env.PlatformID)] $($ie_env.PlatformName) -- OS: $($ie_env.OS)/$($ie_env.OSVer)" }
        $ie_env.fileName = "$($ie_env.PlatformID)_$($ie_env.architecture)_$($ie_env.OS.Substring(3)).0.$($ie_env.OSVer).xml"
    } catch {
        Write-Error $error[0].exception                       # $error[0].exception.gettype().fullname
    ...
    try {
        if ( -not $CleanOutput ) { TraceLog -Message "-- Obtaining Softpaq List" }
        ...
        if ( -not $CleanOutput ) { TraceLog -Message "-- Working File: $($ie_env.XMLFilePath)" }
    } catch {
...
Function Get-ReferenceFileDeviceDriverInfo {
    ...
    if ( $DebugOut ) { TraceLog -Message "      > Get-ReferenceFileDriverInfo() - Search <Devices> for a match to:$($pDeviceToMatch)" }
    ...
    if ( $null -eq $gr_RefernceFileDeviceDriverInfo.RefFileDriverVersion ) { 
        foreach ( $gr_iXMLRefFileSolution in $Script:XMLRefFileSolutions.UpdateInfo ) {
            if ( $gr_iXMLRefFileSolution.ID -like $pSoftpaqID ) {
                ...
                if ( $DebugOut ) { TraceLog -Message "      <XML Reference File <Solution> - Found matching driver version" }
                break
            } # if ( $gr_iXMLRefFileSolution.ID -like $pSoftpaqID )
        } # foreach ( $gr_iXMLRefFileSolution in $Script:XMLRefFileSolutions.UpdateInfo )
    } # if ( $null -eq $gr_iXMLRefFileSolution.Version )
    
    if ( $DebugOut ) { TraceLog -Message "      < Get_ReferenceFileDriverInfo() - matched $($gr_RefernceFileDeviceDriverInfo.HardwareID)/$($gr_RefernceFileDeviceDriverInfo.RefFileDriverVersion)" }
...
Function Get-HardwareMatch {
    ...
    if ( $DebugOut ) { TraceLog -Message '   > Get-HardwareMatch() - Search PnP Drivers for a match' }
    ...
    foreach ($gh_iEntry in $gh_MatchedRefFileDeviceList) {                    
        ...
        foreach ( $gh_iPnPDriver in $pPnPDrivers ) {
            ...
            if ( $gh_iPnPDriver.DeviceID -match "$gh_pattern" ) {
                if ( $DebugOut ) { TraceLog -Message "      matching $($gh_iEntry.DeviceID) with this pattern: {$($gh_pattern)}" }                    
                if ( $DebugOut ) { TraceLog -Message "      matched PnP DeviceID $($gh_iPnPDriver.DeviceID) - $($gh_iPnPDriver.DriverVersion)" }                    
    ...
    if ( $DebugOut ) {
        if ( $gh_HardwareInfo.HardwareID ) {
            TraceLog -Message "   < Get-HardwareMatch() - returning"
            TraceLog -Message "   ... PnP HWID matched: $($gh_HardwareInfo.HardwareID)"
            TraceLog -Message "   ... PnP Driver version: $($gh_HardwareInfo.PnPDriverVersion)"
            TraceLog -Message "   ... PnP Driver Date: $($gh_HardwareInfo.PnpDriverDate)"
            TraceLog -Message "   ... PnP Driver Class: $($gh_HardwareInfo.PnpDriverClass)"
        } else {
            TraceLog -Message '   < Get-HardwareMatch() PnP driver NOT matched'
...
Function Get-DetailFileInfo {
    ...
    #>
    if ( $DebugOut ) { TraceLog -Message "     > Get-DetailFileInfo() - search for $($pFileDetail.FileName)" }    
    ...
    $pFileDetail | ForEach-Object {
        ...
        $gd_ExpandedPath = Expand-CVAPath $_.Directory
        $gd_FilePath = Join-Path -Path $gd_ExpandedPath -ChildPath $_.FileName
        $gd_FileName = $_.FileName

        if ( Test-Path $gd_FilePath ) {        
            $gd_FileInfo.FullPath = $gd_FilePath
            $gf_VersionInfo = (Get-Item $gd_FilePath).VersionInfo
            $gd_FileInfo.ProductVersion = $gf_VersionInfo.ProductVersion
            if ( $gd_FileInfo.ProductVersion -notmatch '_\d+\.\d+\.\d+\.\d+$' ) { $gd_FileInfo.ProductVersion = $gf_VersionInfo.FileVersion}
            # fix when version info includes more than the version and when the version contains ',' instead of '.'
            $gd_FileInfo.ProductVersion = $gd_FileInfo.ProductVersion.split(' ')[0].replace(',','.') # handle bad File Version entry
            if ( $DebugOut ) { TraceLog -Message "       [DetailFile] file found: $($gd_FilePath)/$($gd_FileInfo.ProductVersion)" }
            $gd_Details += $gd_FileInfo             # add to the list of found entries
        } else {
            # search for the driver file in the parent path, in case the location is updated for every version
            $gf_parentFolder = (Split-Path $gd_ExpandedPath -Parent)
            if ( $DebugOut ) { TraceLog -Message "       Searching for $($gd_FileName) from parent folder $($gf_parentFolder)" }        
            $gf_FilePaths = Get-ChildItem -Path $gf_parentFolder -Recurse -File -ErrorAction SilentlyContinue | 
                Where-Object { $_.Name -eq $gd_FileName } #| Select-Object -First 1
            if ( $DebugOut ) { TraceLog -Message "       Found $($gf_FilePaths.Count) files matching $($gd_FileName) in parent folder $($gf_parentFolder)" }    

            # there are multiple files matching the name starting with the parent folder
            # try to sort by ProductVersion if populated and return the most recent
            if ($gf_FilePaths.Count -gt 0) {

                $gf_fileDetails = foreach ($file in $gf_FilePaths) {
                    $fileInfo = Get-Item $file.FullName
                    $versionInfo = $fileInfo.VersionInfo
                    if ( $versionInfo.ProductVersion -match '_\d+\.\d+\.\d+\.\d+$' ) {
                        [PSCustomObject]@{ FullPath = $fileInfo.FullName ; ProductVersion = $versionInfo.ProductVersion }                                   
                    } else {
                        [PSCustomObject]@{ FullPath = $fileInfo.FullName ; ProductVersion = $versionInfo.FileVersion }
                    }
                } # foreach ($file in $gf_FilePaths)

                # Try to sort by ProductVersion if populated
                $gf_sortedFiles = $gf_fileDetails | Sort-Object -Property @{
                    Expression = { 
                        if ([string]::IsNullOrEmpty($_.ProductVersion)) { 
                            [version]"0.0.0.0" 
                        } else { 
                ...
                $gd_FileInfo.FullPath = $gf_sortedFiles[0].FullPath
                $gd_FileInfo.ProductVersion = $gf_sortedFiles[0].ProductVersion

            } # if ($gd_FileInfo.Count -gt 0)    

            if ( $DebugOut ) { TraceLog -Message "       [DetailFile] file found: $($gd_FileInfo.FullPath)/$($gd_FileInfo.ProductVersion)" }
            $gd_Details += $gd_FileInfo             # add to the list of found entries

        } # else if ( Test-Path $gd_FilePath )

    } # $pFileDetail | ForEach-Object

    # returns a list of found entries pointing to each driver file found
    # most will be only one entry, but some may have multiple entries
    return $gd_Details          

} # Function Get-DetailFileInfo

<######################################################################################
    Function Get-DetailFileDriverInfo
    This function returns the driver version string from the CVA file's [DetailFileInformation] section
...
Function Get-DetailFileDriverInfo {
    ...
    if ( $DebugOut ) { TraceLog -Message "   > Get-DetailFileDriverInfo() - searching for $($pSolution.Id)" }    
    ...
    $gd_FileDetail = $pSolution.DetailFileInformation.FileDetail | Where-Object {$_.OS -eq "$($pOS)_$($pOSVer)"}
    if ( $DebugOut ) { TraceLog -Message "   ... search in: $($gd_FileDetail.Directory) for $($gd_FileDetail.FileName)/$($gd_FileDetail.Version)" }
    <#
        <FileDetail>
          <FileName>igdkmdn64.sys</FileName>
          <Directory>&lt;WINSYSDIR&gt;\DriverStore\FileRepository\iigd_dch.inf_amd64_e35423a455e0d784\</Directory>
    ...
    #>
    if ( $null -ne $gd_FileDetail ) {
        # returns an array of matching entries
        $gd_GetDetailFileinfo = Get-DetailFileInfo $gd_FileDetail
        if ( $gd_GetDetailFileinfo.Count -gt 0 ) {
            # get the first entry in the list of matching entries
            #$gd_GetDetailFileinfo = $gd_GetDetailFileinfo[0]        # <<< NEED TO DEAL WHEN MORE THAN 1 ENTRY RETURNED >>>
            $gd_GetDetailFileinfo = $gd_GetDetailFileinfo | Select-Object -First 1
            $gd_DetailFileHash.DetailFileSoftpaqVersion = $gd_FileDetail.Version
            $gd_DetailFileHash.DetailFileInstalledVersion = ($gd_GetDetailFileinfo.ProductVersion)
            if ( $DebugOut ) { TraceLog -Message "   ... found: $($gd_GetDetailFileinfo.FullPath)/$($gd_GetDetailFileinfo.ProductVersion)" }
        } # if ( $gd_GetDetailFileinfo.Count -gt 0 )
    } # if ( $null -ne $gd_FileDetail )

    if ( $DebugOut ) {
        TraceLog -Message "   < Get-DetailFileDriverInfo()"
        TraceLog -Message "     ... DetailFile Version/Installed Version: $($gd_DetailFileHash.DetailFileSoftpaqVersion)/$($gd_DetailFileHash.DetailFileInstalledVersion)"  
    } # if ( $DebugOut )
...
Function Get-UWPInfoFromRefFile {
    ...
    if ( $DebugOut ) { TraceLog -Message "   > Get-UWPInfoFromRefFile(): $($pSoftpaq.ID) - Checking for UWP appx" }
    ...
    foreach ( $iUWPEntry in $pLists.XMLRefFileUWPApps ) {
        ...
        if ( $pSoftpaq.ID -like $iUWPEntry.Solutions.UpdateInfo.IdRef ) {    
            if ( $DebugOut ) { TraceLog -Message "   .. Get-UWPInfoFromRefFile(): matched $($pSoftpaq.ID)" }
            ...
            if ( $gu_InstalledAppx ) {
                if ( $DebugOut ) { TraceLog -Message "   ... found: $($gu_InstalledAppx.Name)" }
                ...
                $gu_MatchedRefFileUWPEntry = @{ 
                    ...
                    if ( $DebugOut ) { TraceLog -Message "   ... Installed package name: $($gu_InstalledAppx.PackageFullName)" }
                    if ( $DebugOut ) { TraceLog -Message "   ... Installed UWP name: $($gu_MatchedRefFileUWPEntry.InstalledUWPName)/$($gu_MatchedRefFileUWPEntry.InstalledUWPVersion)" }
                ...
                if ( $DebugOut ) { TraceLog -Message "   ... reference file/installed appx version: $($gu_MatchedRefFileUWPEntry.RefFileUWPVersion)/$($gu_MatchedRefFileUWPEntry.InstalledUWPVersion)" }
                if ( $DebugOut ) { TraceLog -Message "   ... Installed UWP status: $($gu_MatchedRefFileUWPEntry.Status)" }
    ...
    if ( $DebugOut ) { TraceLog -Message "   < Get-UWPInfoFromRefFile(): $($pSoftpaq.ID) - Found $($gu_MatchedRefFileUWPList.Count) UWP(s)" }
...
<######################################################################################
    ...
    return: $fd_FindDriverReturn = @{
        MatchedHardwareID=$null
        PnPDriverVersion=$null        
        PnPDriverDate=$null
        DeviceClass=$null
        RefFileDriverVersion=$null
        DetailFileInstalledVersion=$null    # by Get-DetailFileDriverInfo()
...
Function Find-Driver {
    ...
    if ( $DebugOut ) { TraceLog -Message '  > Find-Driver()' }
    ...
    if (  $fd_HardwareIDCheck.HardwareID ) {
        $fd_FindDriverReturn.MatchedHardwareID = $fd_HardwareIDCheck.HardwareID
        $fd_FindDriverReturn.DeviceClass = $fd_HardwareIDCheck.PnpDriverClass
        $fd_FindDriverReturn.PnPDriverVersion = $fd_HardwareIDCheck.PnPDriverVersion
        $fd_FindDriverReturn.PnPDriverDate = $fd_HardwareIDCheck.PnpDriverDate        
        <#
        ...
        #>
        $fd_RefFileDriver = Get-ReferenceFileDeviceDriverInfo $pSoftpaqID $pLists.XMLRefFileDevices $fd_HardwareIDCheck.HardwareID 
        $fd_FindDriverReturn.RefFileDriverVersion = $fd_RefFileDriver.RefFileDriverVersion
        $fd_FindDriverReturn.RefFileDriverDate = $fd_RefFileDriver.RefFileDriverDate

        if ( [version]$fd_FindDriverReturn.PnPDriverVersion -lt [version]$fd_FindDriverReturn.RefFileDriverVersion ) {
            $fd_FindDriverReturn.Status = $Script:Config.Constants.SPQUPDATEAVAILABLE
        } else { 
            $fd_FindDriverReturn.Status = $Script:Config.Constants.SPQUPTODATE
        } # else if ( [version]$fd_FindDriverReturn.PnPDriverVersion -lt [version]$fd_FindDriverReturn.RefFileDriverVersion )
    } # if (  $fd_HardwareIDCheck.HardwareID )
    ...
    $fd_DetailFileInfo = Get-DetailFileDriverInfo $fd_Solution $AnalyzerEnv.OS $AnalyzerEnv.OSVer
    $fd_FindDriverReturn.DetailFileInstalledVersion = $fd_DetailFileInfo.DetailFileInstalledVersion
    <#
        Get-DetailFileDriverInfo returns
            $fd_FindDriverReturn.PnPDriverVersion             # from PnP driver list
            $fd_FindDriverReturn.RefFileDriverVersion         # from Reference file
            $fd_FindDriverReturn.DetailFileInstalledVersion   # from installed driver file
    #>
    if ( $fd_FindDriverReturn.DetailFileInstalledVersion ) { 
        # handle file version than contains date along with the version
        $fd_FindDriverReturn.DetailFileInstalledVersion = $fd_FindDriverReturn.DetailFileInstalledVersion.split(' ')[0]
        if ( [version]$fd_FindDriverReturn.DetailFileInstalledVersion -lt [version]$fd_FindDriverReturn.RefFileDriverVersion ) {
            $fd_FindDriverReturn.Status = $Script:Config.Constants.SPQUPDATEAVAILABLE
        } else { 
            $fd_FindDriverReturn.Status = $Script:Config.Constants.SPQUPTODATE 
        } # else if ( [version]$fd_FindDriverReturn.DetailFileInstalledVersion -lt [version]$fd_FindDriverReturn.RefFileDriverVersion )
    } else {
        $fd_FindDriverReturn.Status = $Script:Config.Constants.SPQUPDATEAVAILABLE       # DetailFile is not installed (so a Softpaq was not installed on this system)
    } # else if ( $fd_FindDriverReturn.DetailFileInstalledVersion )

    # if PnP driver is present, and (perhaps) WU updated it and is newer or equal to the reference file version, then we are up to date
    if ( $fd_FindDriverReturn.PnPDriverVersion ) { 
        if ( [version]$fd_FindDriverReturn.PnPDriverVersion -ge [version]$fd_FindDriverReturn.RefFileDriverVersion ) {
            $fd_FindDriverReturn.Status = $Script:Config.Constants.SPQUPTODATE 
        } # else if ( [version]$fd_FindDriverReturn.PnPDriverVersion -gt [version]$fd_FindDriverReturn.RefFileDriverVersion )
    } # if ( $fd_FindDriverReturn.PnPDriverVersion )

    if ( $DebugOut ) {   # Realtek audio driver - for testing
        TraceLog -Message "  < Find-Driver() - returning"
        TraceLog -Message "     ... HardwareID: $($fd_FindDriverReturn.MatchedHardwareID)"
        TraceLog -Message "     ... PnP DriverVersion: $($fd_FindDriverReturn.PnPDriverVersion)"
        TraceLog -Message "     ... Installed DriverDate: $($fd_FindDriverReturn.PnPDriverDate)"
        TraceLog -Message "     ... Reference File/Installed Driver Versions: $($fd_FindDriverReturn.RefFileDriverVersion) / $($fd_FindDriverReturn.DetailFileInstalledVersion)"
        TraceLog -Message "     ... Status: $($fd_FindDriverReturn.Status)"  
        TraceLog -Message "     ... DeviceClass: $($fd_FindDriverReturn.DeviceClass)"     
    } # if ( $DebugOut )
...
Function Find-Software {
    ...
    if ( $DebugOut ) { TraceLog -Message "   > Find-Software() - Checking Software: $($pSoftpaq.Id)/$($pSoftpaq.name)" }   
    ...
    # let's get the DetailFile info
    $fs_DetailFileInfo = Get-DetailFileDriverInfo $fs_Solution $AnalyzerEnv.OS $AnalyzerEnv.OSVer
    ...
    foreach ( $iInst in $pList.InstalledWOWApps ) {   
        if ( $iInst.DisplayName -match $pSoftpaq.name ) {        
            ...
            if ( $DebugOut ) { TraceLog -Message "      - matched WOW App $($pSoftpaq.Id)/$($fs_ReturnHashTable.name)/$($fs_ReturnHashTable.Version)/$($fs_ReturnHashTable.Date) in `$pInstalledWOWApps" }
            break
        } # if ( $iInst.DisplayName -match $pSoftpaq.name )
    ...
    if ( $fs_ReturnHashTable.Count -eq 0 ) {
        ...
        foreach ( $iInst in $pList.InstalledApps ) {      
            if ( $iInst.DisplayName -match $pSoftpaq.name ) {
                ...
                $fs_ReturnHashTable.Version = $iInst.DisplayVersion.split(' ')[0]
                $fs_ReturnHashTable.Date = $iInst.InstallDate
                if ( $DebugOut ) { TraceLog -Message "      - matched App $($pSoftpaq.Id)/$($fs_ReturnHashTable.Name)/$($fs_ReturnHashTable.Version)/$($fs_ReturnHashTable.Date) in `$pInstalledSoftware" }
                break
            } # if ( $iInst.DisplayName -match $pSoftpaq.name )
    ...
    if ( $DebugOut ) { TraceLog -Message "      - matched apps count: $($fs_ReturnHashTable.Count)" }
    ...
    if ( $DebugOut ) { 
        if ( $fs_ReturnHashTable.Count -gt 0 ) {
            TraceLog -Message "   < Find-Software()"
            TraceLog -Message "     ... Name: $($fs_ReturnHashTable.Name)"
            TraceLog -Message "     ... Reference File Version: $($fs_ReturnHashTable.SoftpaqVersion)"
            TraceLog -Message "     ... Installed Version: $($fs_ReturnHashTable.Version)"
            TraceLog -Message "     ... Status: $($fs_ReturnHashTable.Status)"
        } else {
            TraceLog -Message "   < Find-Software() - NOT installed as Software app"
...
function Resolve-BIOS {
    ...
    if ( $DebugOut ) { TraceLog -Message "  ... Resolve-BIOS() Installed BIOS: $($rb_InstalledBIOS)" }
    ...
    if ( $DebugOut ) { TraceLog -Message "  ... Resolve-BIOS() BIOS Check Status: $($rb_Status)" }
...
function Resolve-Driver {
    ...
    if ( $DebugOut ) { TraceLog -Message "  Resolve-Driver() Softpaq Status: $($pSoftpaqEntry.Status)" }
    ...
    if ( $pSoftpaqEntry.UWP -eq $true ) {
        ...
        foreach ( $i_UWP in $ad_UWPAppxInfo ) { # in case the Softpaq has > 1 UWP/Appx applications
            ...
            if ( $pSoftpaqEntry.UWPStatus -eq $Script:Config.Constants.SPQUWPUPDATEAVAILABLE ) { 
                if ( $DebugOut ) {                                
                    TraceLog -Message "  Analyze(): SOFTPAQ NEEDS UPDATE DUE TO UWP: $($ad_result.UWPName)"
                    TraceLog -Message "  ... UWP Available/Installed Version: $($ad_result.UWPVersion)/$($ad_result.UWPInstallVersion)"
                    TraceLog -Message "  ... UWP Status: $($ad_result.UWPStatus)"
                } # if ( $DebugOut )
                break   
        ...
        if ( $DebugOut ) { TraceLog -Message "  Analyze() SOFTPAQ DOES NOT INCLUDE A UWP" }
...
function Resolve-Software {
    ...
    if ( $pSoftpaq.UWP -eq $true -or ($pSoftpaq.Category -match 'Utility') ) {
        ...
        if ( $as_UWPInfoSoftware ) {
            if ( $DebugOut ) { TraceLog -Message "  Resolve-Software(): UWP: $($as_UWPInfoSoftware.InstalledUWPName)/$($as_UWPInfoSoftware.RefFileUWPVersion)/$($as_UWPInfoSoftware.InstalledUWPVersion)/$($as_UWPInfoSoftware.Status)" }
...
function Resolve-Diagnostics {
    ...
    $pSoftpaqEntry = Initialize-AnalysisResult $pSoftpaq 'Diagnostic'

    $rd_Solution =  $pLinks.XMLRefFileSolutions.UpdateInfo | Where-Object {$_.id -eq $pSoftpaq.id}
    # let's get the DetailFile info
    $rd_DetailFileInfo = Get-DetailFileDriverInfo $rd_Solution $AnalyzerEnv.OS $AnalyzerEnv.OSVer
...
<######################################################################################
    Function Initialize-AnalysisResult
...
function Test-StringContainsArrayItems {
    ...
    foreach ($item in $SearchArray) {
        $contains = $InputString.ToLower().Contains($item.ToLower())
        if ($contains) {
            return $true
        }
    } # foreach ($item in $SearchArray)
...
Function Invoke-SoftpaqAnalysis {
    ...
    if ( $DebugOut ) { TraceLog -Message "  > Analyze() $($pSoftpaq.id): $($pSoftpaq.Category)" }

    # Handle Dock associated drivers that are Category 'All Docks' just like Firmware Softpaqs
    # treat them as Categry Driver
    if ( $pSoftpaq.Category -match '^Dock' -and ($pSoftpaq.Name -match '^All Docks') ) {
        $pSoftpaq.Category = 'Driver'
        if ( $DebugOut ) { TraceLog -Message "  ... Softpaq Category changed to Driver for the analysis" }
    } # if ( $pSoftpaq.Category -eq 'All Docks' )

    switch -regex ( $pSoftpaq.Category) {
        'bios' {
            if ( Test-CategoryFilter 'bios' $pCategory ) {
                $is_SoftpaqHashEntry = Initialize-AnalysisResult $pSoftpaq 'BIOS'    # set the default values
        ...
        '^driver' {
            if ( Test-CategoryFilter 'driver' $pCategory ) {
                if ( $Script:SubCategory ) {
                    ...
                    if ( -not $is_SubTest ) {
                        if ( $DebugOut ) { TraceLog -Message " - Softpaq $($pSoftpaq.Id) Category: $($pSoftpaq.category), doesn't match driver type check, skipping." }
                        continue
                    } # if ( -not $is_SubTest )
                } # if ( $Script:SubCategory )
                $is_SoftpaqHashEntry = Initialize-AnalysisResult $pSoftpaq 'Driver'
        ...
        'software|utility' {
            if ( -not $pCategory ) {
                $is_SoftpaqHashEntry = Initialize-AnalysisResult $pSoftpaq 'Software'
        ...
        'diagnostic' {
            if ( -not $pCategory ) {
                $is_SoftpaqHashEntry = Initialize-AnalysisResult $pSoftpaq 'Diagnostic'
...
Function initialize-HPIArepository {
    ...
    if ( -not (Test-Path $pRepoFolder) ) {    
        Try {
            ...
            if ( -not $CleanOutput ) { TraceLog -Message "  ... problem: $($_)" -Type $TypeError }
            return $false
...
Function initialize-DownloadFolder {
    ...
    if ( $DebugOut ) { TraceLog -Message "  > initialize-DownloadFolder() checking download folder: $pDownloadFolder" }
...
<######################################################################################
    ...
    This function downloads the Softpaq executable, CVA file, and HTML release notes
    parm: $pSoftpaqObj:    the Softpaq hash entry to download
           $pFolderPath:    the folder path to download to
    return: none
#>#####################################################################################
Function Get-SoftpaqFiles {
    param( $pSoftpaqObj, $pFolderPath, $pDownloadAction )

    $gs_CurrLocation = Get-Location
    Set-Location $pFolderPath

    $gs_DownloadPath = $pFolderPath+'\'+$pSoftpaqObj.SoftpaqID
    $gs_ExePath = $gs_DownloadPath+'.exe'

    if ( $DebugOut ) { TraceLog -Message "... downloading Softpaq $($pSoftpaqObj.SoftpaqID) - $($pSoftpaqObj.SoftpaqName)" -Type $TypeNorm }
    if ( Test-Path $gs_ExePath ) {
        if ( $DebugOut ) { TraceLog -Message "`t$($pSoftpaqObj.SoftpaqID) already downloaded - $($pSoftpaqObj.SoftpaqName)" -Type $TypeWarn }
    } else {
        if ( $DebugOut ) { TraceLog -Message "`tdownloading $($pSoftpaqObj.SoftpaqID)/$($pSoftpaqObj.SoftpaqName)" -Type $TypeNoNewline }
        Try {
            (Get-Softpaq $pSoftpaqObj.SoftpaqID) 6>&1
            if ( $DebugOut ) { TraceLog -Message "... done" -Type $TypeNorm }            
        } Catch {
            if ( $DebugOut ) { TraceLog -Message "... failed to download: $($_)" -Type $TypeError }
        } # Try/Catch block for Get-Softpaq
    } # else if ( Test-Path $ds_ExePath ) {
<#
    if ( $pDownloadAction -eq 'Extract' ) {
        # Extract the Softpaq contents to the specified folder, includes the SoftpaqID in the path
        if ( $DebugOut ) { TraceLog -Message "`textracting Softpaq" -Type $TypeNorm }
        if ( -not (Test-Path $gs_DownloadPath) ) {
            Try {
                $ie_newDir = New-Item -Path $gs_DownloadPath -ItemType directory | Out-Null
                if ( -not $CleanOutput ) { TraceLog -Message "  ... extract path was not found, created: $gs_DownloadPath" -Type $TypeNorm }
            } Catch {
                if ( -not $CleanOutput ) { TraceLog -Message "... failed to create extract path: $($_)" -Type $TypeError }
            } # try/catch block for New-Item
        } # if ( -not (Test-Path $gs_DownloadPath) )        
        $gs_ExtractArgs = "/s /e /f `"$gs_DownloadPath`""
        try { 
            $gs_ExtractCode = Start-Process -FilePath $gs_ExePath -ArgumentList $gs_ExtractArgs -Wait -NoNewWindow -PassThru 
        } Catch { 
            if ( $DebugOut ) { TraceLog -Message "... extract return code: $($gs_ExtractCode)" -Type $TypeError }
        }        
    } # if ( $pDownloadAction -eq 'Extract' )
#>
    # download corresponding CVA file
    if ( $DebugOut ) { TraceLog -Message "`tdownloading $($pSoftpaqObj.SoftpaqID) CVA file" -Type $TypeNoNewline }
    Try {
        Get-SoftpaqMetadataFile $pSoftpaqObj.SoftpaqID -Overwrite 'Yes' 6>&1   # ALWAYS update the CVA file, even if previously downloaded
        if ( $DebugOut ) { TraceLog -Message "... done" -Type $TypeNorm }
    } Catch {
        if ( $DebugOut ) { TraceLog -Message "... failed to download: $($_)" -Type $TypeError }
    } # Try/Catch block for Get-SoftpaqMetadataFile
    
    # download corresponding HTML file
    if ( $DebugOut ) { TraceLog -Message "`tdownloading $($pSoftpaqObj.SoftpaqID) Release Notes file" -Type $TypeNoNewline }
    Try {
        $ds_SoftpaqHtml = $pFolderPath+'\'+$pSoftpaqObj.SoftpaqID+'.html' # where to download to
        $ds_htmlURL = $pSoftpaqObj.url.replace('exe','html')
        Invoke-WebRequest -UseBasicParsing -Uri "$($ds_htmlURL)" -OutFile "$($ds_SoftpaqHtml)"
    } Catch {
        if ( $DebugOut ) { TraceLog -Message "... failed to download" -Type $TypeError }
    } # Try/Catch block for Invoke-WebRequest

    Set-Location $gs_CurrLocation
...
<######################################################################################
    ...
    parm: $pEntryList:     the list of Softpaqs to process
           $pGetAction:     the action to perform (Scan, Download, Install)
...
Function Get-AllSoftpaqs {
    ...
    $ga_count = 0
    foreach ($entry in $pEntryList) {
        if ( $entry.CVAHWID -notlike 'NA' ) {
            $ga_count++
            Get-SoftpaqFiles -pSoftpaqObj $entry -pFolderPath $Script:ActionPath -pDownloadAction $pGetAction
        } # if ( $entry.CVAHWID -notlike 'NA' )
    } # foreach ($entry in $pEntryList)
...
Function Get-OutputLine {
    ...
    if ( $pEntry.InstallVersion ) {
        if ( $pEntry.InstallVersion -like 'MISSING' ) {
            ...
            if ( $pEntry.CVAHWID -like 'NA' ) {           # PnP ID Not found in systems
...
#######################################
# do some basic options conflict checks
#######################################
...
if ( $SubCategory ) { 
    ...
    if ( -not $Category ) { 
        ...
        if ( -not $CleanOutput ) { TraceLog -Message "-- SubCategory specified with no -Category, setting Category to 'Driver' for reporting" }
        $Category = 'Driver'
    } # if ( -not $Category )
} # if ( $SubCategory )
...
# completed options conflict checks
if ( $DebugOut ) { TraceLog -Message "-- Debug mode enabled" -Type $TypeNorm }

# initialize things for actions
# -----------------------------------------------------------------------------------
$Script:AnalyzerEnv = Initialize-Environment

# make sure the CSV file is not locked - probaly to Excel having it open
try {
    if ( Test-Path $Script:Config.Paths.CsvFile ) { [IO.File]::OpenWrite((Get-ChildItem $Script:Config.Paths.CsvFile).FullName).close() }        
} catch {
    TraceLog -Message "-- CSV File $($Script:Config.Paths.CsvFile) is locked, can not overwrite"
}

switch ($Action) {
    'Scan' {
        if ( -not $CleanOutput ) { TraceLog -Message "-- Analyzing device and generating Report in $($Script:ActionPath)" }
    } # 'Scan'
    'Download' {
        if ( -not $CleanOutput ) { TraceLog -Message "-- Initializing Download Folder at $($Script:ActionPath)" }
        initialize-DownloadFolder $Script:ActionPath
    } # 'Download'
    'CreateRepo' {
        if ( -not $CleanOutput ) { TraceLog -Message "-- Initializing HP Image Assistant Repository at $($Script:ActionPath)" }
...
# done initializing things for actions
# -----------------------------------------------------------------------------------
...
# Get the list of PnP signed drivers installed in the system
# limit the list to: drivers that have a version, are not provided by Microsoft, and are not software components
# Note: DeviceID is not null, and does not start with '{' (a GUID)
if ( -not $CleanOutput ) { TraceLog -Message '-- Obtaining PnP Driver list ...' }
...
if ( -not $CleanOutput ) { TraceLog -Message '-- Analyzing Softpaqs - Please wait...' }

foreach ( $Spq in $AnalyzerEnv.SoftpaqList ) {

    if ( -not $NoDots ) { if ( -not $CleanOutput ) { TraceLog -Message "." -Type $TypeNoNewline } } # if ( -not $NoDots )
    if ( $DebugOut ) { TraceLog -Message "-- Analyzing Softpaq: $($Spq.id) - $($Spq.name)" }    
    
    # manage Softpaqs we do not want to analyze
    if ( $Spq.Category -match '^Manageability' ) {
        if ( $DebugOut ) { TraceLog -Message "  ... skipping Softpaq: $($Spq.id) - $($Spq.name) - Category ''manageability'' not supported by script" }
        continue
    } # if ( $Spq.Category -match '^Manageability' )
    if ( $Spq.Category -match '^Dock' -and ($Spq.Name -notmatch '^All Docks') ) {
        if ( $DebugOut ) { TraceLog -Message "  ... skipping Softpaq: $($Spq.id) - $($Spq.name) - Category 'Dock' firmware not supported by script" }
        continue
    } # iif ( $Spq.Category -match '^Dock' -and ($Spq.Name -notmatch '^All Docks') )

    if ( $spq.name -match '^HP Wolf|^HP Sure' ) {
        if ( $DebugOut ) { TraceLog -Message "  ... skipping Softpaq: $($Spq.id) - $($Spq.name) - is managed" }
        continue
    } # if ( $spq.name -match '^HP Wolf|^HP Sure' )

    # handle Software Softpaqs that are HP managed - avoid analyzing them

    # Let's do the analysis
    $SoftpaqEntry = Invoke-SoftpaqAnalysis $Spq $PnpSignedDrivers $AnalyzerEnv.Links $Category
...
####################################################################
if ( -not $NoDots ) {if ( -not $CleanOutput ) { TraceLog -Message ' ' } }         # output empty line to separate output
...
if ( -not $CleanOutput ) { TraceLog -Message '-- Softpaq Updates' }
foreach ( $r in $SoftpaqsUpdateList ) {    
    if ( $r.Category -notmatch 'software' ) {           # Drivers/BIOS first
        if ( $r.CVAHWID -notlike 'NA' ) {               # don't show it the component is NOT on the PnP Drivers list in the system
            $TotalRemediations++
            TraceLog -Message (Get-OutputLine $r $ShowHWID)
            $JsonOut.Remediations += New-JsonItem $r    # create JSON output for the remediation
        } # if ( $r.CVAHWID -notlike 'NA' )        
...
foreach ( $r in $SoftpaqsUpdateList ) { 
    if ( $r.Category -match 'software' ) {      # output Software last
        $TotalRemediations++
        TraceLog -Message (Get-OutputLine $r $ShowHWID)
        $r.SoftpaqVersion = $r.ReferenceFileVersion     # use the ReferenceFileVersion for Software Softpaqs
        $JsonOut.Remediations += New-JsonItem $r        # create JSON output for the remediation
    } # if ( $r.Category -match 'software' )
} # foreach ( $r in $SoftpaqsUpdateList )

####################################################################
# NEXT: Software Softpaqs NOT installed, but recommended (-r)
####################################################################

if ( $RecommendedSoftware ) {
    if ( -not $CleanOutput ) { TraceLog -Message '' }
    if ( -not $CleanOutput ) { TraceLog -Message '-- Software Updates - NOT Installed but Recommended' }
    $SoftpaqsNOTInstalledList | ForEach-Object {
        if ( $_.SoftpaqName -in $Script:Config.RecommendedSWList ) {
            $TotalRemediations++
            TraceLog -Message (Get-OutputLine $_ $ShowHWID)
            $_.SoftpaqVersion = $_.ReferenceFileVersion     # use the ReferenceFileVersion for Software Softpaqs            
            $JsonOut.Remediations += New-JsonItem $_        # create JSON output for the remediation
            $SoftpaqsUpdateList += $_       # adding to the update list
        } # if ( $r.Category -notmatch 'software' )
    } # $SoftpaqsNOTInstalledList | foreach
} # if ( $RecommendedSoftware )

# -----------------------------------------------------------------------------------

####################################################################
# FINALLY: perform any actions based on the Action specified
####################################################################

switch ( $Action ) {
    'CreateRepo' {
        if ( -not $CleanOutput ) { TraceLog -Message "-- Downloading Softpaqs to the Repository Folder: $($Script:ActionPath)" }
        $TotalRemediations = Get-AllSoftpaqs -pEntryList $SoftpaqsUpdateList -pGetAction $Action # $Action = 'Scan'|'Download'|'CreateRepo'
    } # 'CreateRepo'
    'Download' {
        if ( -not $CleanOutput ) { TraceLog -Message "-- Downloading Softpaqs to: $($Script:ActionPath)" }
        $TotalRemediations = Get-AllSoftpaqs -pEntryList $SoftpaqsUpdateList -pGetAction $Action # $Action = 'Scan'|'Download'|'CreateRepo'
    } # 'Download'
} # switch ( $Action )

# Export the Softpaq analysis results to a CSV file
if ( -not $CleanOutput ) { TraceLog -Message "-- Exporting results to CSV-formatted file: $($Script:Config.Paths.CsvFile)" }
Invoke-ReportCSV -pEntryList $SoftpaqsUpdateList -pFilePath $Script:Config.Paths.CsvFile
# -----------------------------------------------------------------------------------

# Artifact of PS5.1/ConvertTo-Json that does not handle '&' and '\' characters - converts to Unicode characters
    if ( -not $CleanOutput ) { TraceLog -Message "-- Exporting results to JSON-formatted file: $($Script:Config.Paths.JsonFile)" }
$JsonOut = $JsonOut | 
...
if ( -not $CleanOutput ) { TraceLog -Message "-- Elapsed: [min:sec] $($elapsedTime.ToString("mm\:ss"))" }
