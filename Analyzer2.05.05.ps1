...
# Script metadata
$ScriptVersion = '2.05.04'
$ScriptName = 'Analyzer'
$ScriptAuthor = 'Dan Felman, HP Inc.'
$ScriptLastModified = '2025-08-27'

# Performance tracking
$Script:StartTime = Get-Date
$Script:ExecutionId = (Get-Date).ToString('yyyyMMdd-HHmmss')

Write-Verbose "[$($ScriptName)] Starting execution - Version: $ScriptVersion"
Write-Verbose "[$($ScriptName)] Execution ID: $ExecutionId"
Write-Verbose "[$($ScriptName)] Parameters: Action=$Action, ActionPath=$ActionPath"
if ($Category) { Write-Verbose "[$($ScriptName)] Category filter: $($Category -join ', ')" }
if ($SubCategory) { Write-Verbose "[$($ScriptName)] SubCategory filter: $($SubCategory -join ', ')" }
...
$Script:DebugLogPath = if ($DebugLogEnabled) {
    "$($PSScriptRoot)\Analyzer-$((Get-Date).ToString('yyyyMMdd-HHmm'))-Debug.log"
}

Write-Debug "[$($ScriptName)] Debug logging enabled: $DebugLogEnabled"
if ($DebugLogEnabled) { Write-Debug "[$($ScriptName)] Debug log path: $DebugLogPath" }
...
Write-Verbose "[$($ScriptName)] Configuration of constants initialized"
Write-Debug "[$($ScriptName)] Log file: $($Script:Config.Paths.LogFile)"
Write-Debug "[$($ScriptName)] CSV file: $($Script:Config.Paths.CsvFile)"
Write-Debug "[$($ScriptName)] JSON file: $($Script:Config.Paths.JsonFile)"
