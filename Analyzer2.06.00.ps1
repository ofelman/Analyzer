...
.LINK
    https://developers.hp.com/Analyzer
...
# Script metadata
$ScriptVersion = '2.06.00'
$ScriptName = 'Analyzer'
$ScriptAuthor = 'Dan Felman, HP Inc.'
$ScriptLastModified = '2025-09-04'
...
if ( $Help ) {
    'Analyzer - An HP Business System Update Detection and Update Tool'
    'Usage:' 
    'Analyzer.exe [options]                                 # defaults to ''-Action Scan'''
        ...
        '  [-c|Category <BIOS,Driver>]                      # Limit analysis to these categories: BIOS and/or Driver'
        '       defaults to All'
        '  [-cve|CVECheck]                                  # Check for CVE fixes in Softpaqs that include CVE information'
        '  [-d|Debug]                                       # Output additional script debugging information'
        '  [-h|Help]                                        # Display this help message'
        '  [-r|ReferenceFile <path to .xml file>]           # Use a local XML reference file for analysis'
        '  [-s|SubCategory <Audio,Chipset,Graphics,Network,Video>]   # Filter by driver types'
        '       assumes -Category Driver, even if not in command line'
        '  [-v|Verbose]                                     # Output additional script verbose information'
    return
} # if ( $Help )
...
<#####################################################################################
    Test-CategoryFilter
    This function tests if a given category matches the filter criteria.
    It returns $true if the category matches or if no filter is set, otherwise $false.
    Parameters:
        - $pTest: The test string to match against the category.
        - $pCategory: The category string to test.
#####################################################################################>
...
<#####################################################################################
    Resolve-BIOS
    This function resolves the BIOS update status by comparing the installed BIOS version
    with the version provided in the Softpaq entry. It updates the Softpaq entry with the
    installed version and status.
    Parameters:
        - $pSoftpaqEntry: The Softpaq entry object to update.
        - $pSoftpaq: The Softpaq object containing the version information.
    returns the updated Softpaq entry object.
#####################################################################################>
...
Function ConvertTo-CsvLine {
    [CmdletBinding()]
    param([Parameter(Mandatory)]$pEntry)
    
    # Format driver information if present
    $cs_driverInfo = if ($pEntry.Drivers) {
        ($pEntry.Drivers | ForEach-Object { "$($_.DeviceID);$($_.PnPDriverVersion)/$($_.RefFileDriverVersion)" }) -join '|'
    } else { "" }

    # Create CSV line with essential fields
    return [PSCustomObject]@{
        SoftpaqID = $pEntry.SoftpaqID
        Name = $pEntry.SoftpaqName
        SubCategory = $pEntry.SoftpaqCategory
        Status = $pEntry.SoftpaqStatus
        CurrentVersion = $pEntry.DetailFileInstalledVersion
        AvailableVersion = $pEntry.DetailFileVersion
        ReleaseDate = $pEntry.SoftpaqDate
        UWPName = $pEntry.UWPName
        UWPVersion = $pEntry.UWPInstallVersion
        UWPAvailableVersion = $pEntry.UWPVersion
        Drivers = $cs_driverInfo
        CVEs = if ($pEntry.CVEs) { $pEntry.CVEs -join '|' } else { "" }
        SilentInstall = $pEntry.SilentInstall
        URL = $pEntry.URL        
    }
} # Function ConvertTo-CsvLine()
...
Function Get-CVEListFromCVA {
    ...
    # handle [US.Enhancements] section of CVA file
    if ( $null -ne $pCVAEnhancementsSection ) {
        foreach ( $iLine in $pCVAEnhancementsSection ) {    # check every line under [US.Enhacemenents] for CVE
            $iLine = ($iLine -split '/' ) -join ','   # replace '/' with ',' to handle CVE lists separated by '/'
            $iLine = ($iLine -split ',' ).replace('.','').Trim().split(' ') -match "(CVE-[1-2][0-9]{3}-[0-9]{3,5})"
            $gc_CVEList += $iLine
        } # foreach ( $iLine in $pCVAEnhancementsSection )
    } # if ( $null -ne $pCVAEnhancementsSection )

    # handle [Private_Fixes] section of CVA file
...
$FilteredSoftpaqs | ForEach-Object {
    ...
    if ( $SoftpaqEntry.SoftpaqStatus -eq $Script:Config.Constants.SPQUPDATEAVAILABLE ) {
        ...
        switch ( $Action ) {
            'Scan' {
                # Analysis is done below after the switch statement
                # Let's add a CVE check here
                if ( $Script:CVECheck ) { 
                    Try {
                        $Error.Clear()
                        $SpqMetadata = Get-SoftpaqMetadata $CurrentSpq.id -ErrorAction Continue
                        $SoftpaqEntry.CVEs = Get-CVEListFromCVA $SpqMetadata.'US.Enhancements'._body $SpqMetadata.'Private_Fixes'._body
                        if ( $SoftpaqEntry.CVEs ) {
                            #$SoftpaqEntry.CVEs = $SoftpaqEntry.CVEs -join ', '
                            Add-Verbose -Message "   ... found CVEs: $($SoftpaqEntry.CVEs -join ', ')" 
                        }  # if ( $SpqCVEs )
                    } Catch {
                        Add-Verbose -Message "Failed to retrieve metadata for Softpaq $($CurrentSpq.id)"
                    } 
                }  # skip CVE check if not requested
...
$SoftpaqsUpdateList | ForEach-Object {
    $msg = "$($_.SoftpaqID) - $($_.SoftpaqName) <$($_.SoftpaqCategory)> - $($_.SoftpaqVersion) [DetailFile/installed:$($_.DetailFileVersion)/$($_.DetailFileInstalledVersion)] - Action/Status: $($_.Action)/$($_.ActionReturnCode)"
    Add-ToLog -Message $msg

    # Export to  CSV and set-up JSON output
    $softpaqData = ConvertTo-CsvLine -pEntry $_
